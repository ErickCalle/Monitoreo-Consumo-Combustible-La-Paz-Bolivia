...
Después de cargar el firmware al esp32 y montar el circuito planteado en la Figura \ref{fig:obd_conector_soldado} se procede a obtener tramas CAN, los siguientes son usados en el sistema operativo Ubuntu 22.04 para tener mas control de los comunicadores.
...
A diferencia del modelo tradicional de petición-respuesta de HTTP, en el que el cliente debe solicitar cada actualización de forma individual, el protocolo WebSocket \citep{rfc6455} establece un canal de comunicación bidireccional y persistente sobre una única conexión TCP. Una vez completado el saludo inicial (\textit{handshake}), que sí se realiza sobre HTTP, el servidor puede enviar datos al cliente en cualquier momento sin que este los solicite explícitamente, con un sobrecosto de encabezado mucho menor que el de una petición HTTP completa repetida. Esta característica lo hace especialmente adecuado para la actualización en tiempo real del panel web del presente sistema, donde el ESP32-S3 necesita empujar el estado del vehículo al navegador de forma periódica sin que este tenga que preguntar repetidamente si hay datos nuevos.
...
La base y la tapa, ya impresas (21~g y 16~g de PLA, respectivamente), se muestran en la Figura~\ref{fig:carcasa_3d}. El costo del material se presenta en el Capítulo~5 (Tabla~\ref{tab:costo_carcasa}).
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Images/TAPA_SUPERIOR.jpeg}
		\caption{Tapa (impresa, 16~g)}
		\label{fig:carcasa_3d_tapa}
	\end{subfigure}
	\vspace{2pt}
	\\
	{\footnotesize\textit{Nota.} Impresión propia en PLA. La tapa incorpora el recorte rectangular para la pantalla OLED editado en SolidWorks (Figura~\ref{fig:tapa_editada}). Elaboración propia.}
\end{figure}
...
%  Proyecto de grado: Sistema de monitoreo de consumo de combustible OBD-II / CAN
%
...
%  Proyecto de grado: Sistema de monitoreo de consumo de combustible OBD-II (ESP32 + SN65HVD230)
...
%  Proyecto de grado: Sistema de monitoreo de consumo OBD-II (ESP32 + SN65HVD230)
%  Autor: Erick — Ing. Mecatrónica, UMSA (La Paz, Bolivia)
% ---------------------------------------------------------------------
