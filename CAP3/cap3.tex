...
Tras compilar el código y conectar el sistema al puerto OBD-II del vehículo con el contacto en posición \textit{ON}, el monitor serie arrojó la siguiente salida a $115200\text{ baudios}$ ver Figura \ref{fig:monitor_serie_obd2}:

\begin{figure}[H]
	\caption{Captura de pantalla del monitor serie durante la ejecución del escáner de PIDs OBD-II.}
	...
	\textit{Nota.} Registro de depuración en tiempo real que muestra la inicialización correcta del controlador TWAI, el envío de la solicitud funcional (PID 0x00) y la respuesta hexadecimal (\texttt{FE 3E B8 13}) emitida por la unidad de control del motor (ECU).
...
\subsubsection{Distribución de Pantallas y Navegación}

La información adquirida del bus CAN se organizó en tres pantallas de 
visualización, navegables mediante un pulsador físico conectado al ESP32. 
La distribución de parámetros por pantalla se detalla en la Tabla \ref{tab:distribucion_pantallas}.

\begin{table}[H]
	\centering
	\captionsetup{labelformat=simple, labelsep=newline, textfont=it, labelfont=bf, justification=raggedright, singlelinecheck=false}
	\caption{Distribución de parámetros OBD-II por pantalla OLED}
	...
	\begin{tabular}{p{3.0cm} p{11.2cm}}
		\toprule
		\rowcolor[rgb]{0.851, 0.851, 0.851}
		\textbf{Pantalla} & \textbf{Parámetros mostrados} \\ 
		\midrule
		Pantalla 1 & RPM del motor (PID \texttt{0x0C}), velocidad del vehículo (PID \texttt{0x0D}), carga calculada del motor (PID \texttt{0x04}) y temperatura del líquido de enfriamiento (PID \texttt{0x05}). \\
		
		Pantalla 2 & Consumo instantáneo de combustible, consumo acumulado de la sesión y eficiencia en L/100~km. \\
		
		Pantalla 3 & Ajuste de combustible a corto plazo STFT (PID \texttt{0x06}) y ajuste a largo plazo LTFT (PID \texttt{0x07}). \\
		\bottomrule
	\end{tabular}
	\vspace{0.2cm}
	\begin{flushleft}
		\small
		\begin{center}
			\textit{Nota.} La navegación entre pantallas se implementó mediante interrupción por hardware (ISR) con rutina de antirrebote (\textit{debounce}) por software de 50~ms, evitando lecturas múltiples por un solo accionamiento del pulsador.
		\end{center}
	\end{flushleft}
\end{table}
...
\begin{table}[H]
	\centering
	\captionsetup{labelformat=simple, labelsep=newline, textfont=it, labelfont=bf, justification=raggedright, singlelinecheck=false}
	\caption{Estructura del registro CSV mediante el método Speed-Density (MAP+IAT)}
	...
	\begin{tabular}{p{3.5cm} p{2.0cm} p{8.7cm}}
		...
		\midrule
		\texttt{timestamp\_ms} & uint32 & Tiempo transcurrido desde el arranque del sistema en milisegundos. \\
		\texttt{rpm}           & float  & Revoluciones por minuto del motor (PID \texttt{0x0C}). \\
		\texttt{velocidad}     & uint8  & Velocidad del vehículo en km/h (PID \texttt{0x0D}). \\
		\texttt{map}           & uint8  & Presión absoluta del múltiple de admisión en kPa (PID \texttt{0x0B}). \\
		\texttt{iat}           & int8   & Temperatura del aire de admisión en °C (PID \texttt{0x0F}). \\
		\texttt{temp\_ref}     & int8   & Temperatura del líquido de enfriamiento en °C (PID \texttt{0x05}). \\
		\texttt{stft\_b1}      & float  & Ajuste de combustible a corto plazo, banco 1 en \% (PID \texttt{0x06}). \\
		\texttt{ltft\_b1}      & float  & Ajuste de combustible a largo plazo, banco 1 en \% (PID \texttt{0x07}). \\
		\bottomrule
	\end{tabular}
	\vspace{0.2cm}
	\begin{flushleft}
		\small
		\begin{center}
			\textit{Nota.} El archivo se escribe en modo \textit{append} con un intervalo de muestreo configurable (por defecto 500~ms), registrando de forma síncrona las variables de presión (MAP) y temperatura (IAT) requeridas por la ecuación de estado de los gases ideales para la aproximación del consumo de combustible. Al detectar ausencia del bus CAN, el sistema cierra el archivo de forma segura.
		\end{center}
	\end{flushleft}
\end{table}
...
\begin{table}[H]
	...
	\centering
	\small
	\begin{tabular}{c p{5.9cm} p{3.0cm} c c}
		\toprule
		\rowcolor[rgb]{0.851, 0.851, 0.851}
...
el motor, $\dot{m}_{air}$ [g/s], reportado por el sensor MAF (\textit{Mass Air
	Flow}) a través del PID 0x10 \citep{SAEJ1979_2017}. La estimación de combustible
...
\citep{SAEJ1979_2017}. La convención de codificación es la siguiente: el byte A es
...
\begin{table}[H]
	...
	\vspace{2pt}
	{\footnotesize\textit{Nota.} Codificación según SAE J1979 \citep{SAEJ1979_2017}.
...
\textit{Alcance de la implementación evaluada.} Esta sección formaliza la
\textbf{arquitectura de estimador en lazo cerrado propuesta} para el sistema,
completa con su análisis de estabilidad y su procedimiento de identificación de
parámetros. De ella, el firmware evaluado en el Capítulo~4 implementa
únicamente el corrector directo por ajustes de combustible del ECU
($\mathrm{STFT}+\mathrm{LTFT}$ aplicados sobre el AFR efectivo, descrito en la
Sección~\ref{sec:speeddensity} como parte del \emph{Diseño de software}). El
factor de calibración adaptativo $K$ con filtro EMA
(Subsección~\ref{subsec:factor-k}), la identificación en línea de la curva
$\eta_v(N)$ por regresión (Subsección~\ref{subsec:curva-etav}) y la
conmutación automática MAF/Speed-Density (Subsección~\ref{subsec:soporte_pids})
no están implementados en el prototipo probado; se presentan aquí como el
diseño formal de la extensión y se retoman como recomendaciones concretas de
trabajo futuro en el Capítulo~6. Esta distinción se mantiene explícita a lo
largo de la sección para que el lector no infiera validación experimental de
componentes que aún no se han puesto a prueba.
...
Se eligió el SN65HVD230 \citep{ti2018sn65hvd230} en concreto porque opera
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.2cm}p{4.0cm}p{2.2cm}p{4.5cm}}
		...
		3.3~V (riel 1) & SN65HVD230, siempre activo & $\sim$5–10~mA & Modo normal, no standby \citep{ti2018sn65hvd230} \\
...
\begin{itemize}
	...
	usado en la Tabla~\ref{tab:consumo_sin_contacto} \citep{ti2018sn65hvd230}.
...
%aceptar las respuestas de la ECU \citep{SAEJ1979_2017}. La Figura~\ref{fig:cod-caninit}
