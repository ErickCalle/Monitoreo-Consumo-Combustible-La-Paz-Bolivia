...
\subsubsection{Sistema en chip (SoC): el caso del ESP32-S3}
Un \textit{sistema en chip} (\textit{System on Chip}, SoC) constituye una evolución del microcontrolador clásico: además del núcleo de procesamiento, la memoria y los periféricos digitales típicos, integra en el mismo encapsulado subsistemas de radiofrecuencia (RF) completos, como transceptores de Wi-Fi y Bluetooth, junto con la circuitería analógica de acondicionamiento que estos requieren. Esta integración elimina la necesidad de módulos de comunicación inalámbrica externos, reduce el número de componentes discretos de la placa y el consumo asociado a las interfaces entre chips, a cambio de una mayor complejidad interna del silicio \citep{MikroE2018}.

Un ejemplo representativo de esta arquitectura, y el empleado en el presente proyecto, es el ESP32-S3 de Espressif Systems, en su variante \textbf{ESP32-S3-WROOM-1-N16R8}. Este SoC integra un procesador de doble núcleo Xtensa\textsuperscript{\textregistered}~LX7 de hasta 240~MHz, un controlador de bus CAN nativo (TWAI, \textit{Two-Wire Automotive Interface}) compatible con ISO~11898-1, y radios integrados de Wi-Fi de 2,4~GHz y Bluetooth Low Energy (BLE) 5.0, además de 16~MB de memoria Flash y 8~MB de PSRAM integrados en el propio módulo, de donde proviene la nomenclatura ``N16R8'' (16~MB de Flash NOR, 8~MB de PSRAM) \citep{espressif_esp32_datasheet}. La disponibilidad de un controlador CAN nativo resulta particularmente relevante para este proyecto, ya que permite prescindir de un controlador CAN externo por SPI, como el MCP2515, reduciendo el conteo de componentes de la placa y eliminando una fuente adicional de fallas en la interfaz entre ambos circuitos integrados.
...
En la actualidad, prácticamente todo vehículo moderno incorpora al menos una computadora embebida que gestiona su funcionamiento, independientemente de su tipo de propulsión: los vehículos de combustión interna la emplean para el control del motor y la inyección de combustible, los vehículos híbridos suman a esto la gestión de la transición entre el motor térmico y el eléctrico, y los vehículos eléctricos dependen por completo de ella para el control del motor eléctrico, la batería y la electrónica de potencia. La diferencia entre estas arquitecturas no radica, por tanto, en la presencia o ausencia de electrónica de control, sino en el número, la complejidad y el grado de centralización de dichas unidades de procesamiento, tendencia que se profundiza con la electrificación del tren motriz, tal como se describe a continuación.
