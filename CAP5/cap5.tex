...
Los costos anteriores son costos directos, sin margen de ganancia ni impuestos. La Tabla~\ref{tab:precio_venta} aplica una utilidad del 20\,\%, valor de referencia habitual en la formulación de proyectos de la carrera, y luego el Impuesto al Valor Agregado (IVA) vigente en Bolivia (13\,\%) sobre el precio ya con utilidad, sobre dos bases de costo distintas: el prototipo tal como se construyó (706,92\,Bs, fila ``Total del prototipo'' de la Tabla~\ref{tab:presupuesto_total}) y el costo marginal por unidad adicional si se escalara a la placa final (772,22\,Bs, fila ``Costo marginal estimado por unidad adicional'' de la misma tabla). La diferencia entre ambas cifras es, precisamente, la Tabla~\ref{tab:costo_pruebas} completa (316,32\,Bs: ELM327, tester y la gasolina de las pruebas tanque a tanque): ese gasto sí se incurrió para construir y validar este prototipo, pero no se repetiría al vender una unidad adicional ya validada, así que solo aparece en la columna ``Este prototipo''.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{X >{\centering\arraybackslash}p{3.2cm} >{\centering\arraybackslash}p{3.2cm}}
		\toprule
		\rowcolor[rgb]{0.851, 0.851, 0.851}
		Concepto & Este prototipo (Bs) & Unidad adicional, placa final (Bs) \\
		\midrule
		Costo directo (Tabla~\ref{tab:presupuesto_total}) & 706,92 & 772,22 \\
		Utilidad (20\,\%) & 141,38 & 154,44 \\
		\midrule
		Subtotal (costo + utilidad) & 848,30 & 926,66 \\
		IVA (13\,\%) & 110,28 & 120,47 \\
		\midrule
		\textbf{Precio de venta estimado} & \textbf{958,58} & \textbf{1\,047,13} \\
		\bottomrule
	\end{tabularx}
	\vspace{2mm}
	\\
